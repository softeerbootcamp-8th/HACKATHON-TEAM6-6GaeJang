 ...
 *
 * <p>메뉴 없이 참여하는 경로는 두지 않는다. 총대가 주문을 넣으려면 참여자 메뉴가 반드시 있어야 하고,
 * 참여만 해두고 메뉴를 안 낸 사람이 정원을 차지하면 총대가 최소주문금액을 채울 수 없다.
 *
 * <p>메뉴는 자유 텍스트다. 가게마다 옵션 구조가 달라 서버가 구조화할 수 없고, 총대가 배달앱에
 * 그대로 옮겨 적을 수 있으면 충분하다("허니콤보 세트 (순살로 변경) + 콜라 제로 500ml").
 */
