...
function PotDetailPage() {
  ...
  const detailQuery = useGetPot(potId)
  const detail = detailQuery.data?.data

  if (!Number.isFinite(potId)) return <PageState error="잘못된 배달팟 주소예요." />
  if (detailQuery.isPending) return <PageState>배달팟을 불러오는 중이에요…</PageState>
  if (detailQuery.isError) return <PageState error={detailQuery.error.message} />
  if (!detail) return <PageState error="배달팟 정보를 찾을 수 없어요." />
  if (isEnteringMenu) return <MenuEntry detail={detail} onClose={() => setIsEnteringMenu(false)} />

  return <PotOverview detail={detail} onJoin={() => setIsEnteringMenu(true)} />
}
...
function PotOverview({ detail, onJoin }: { detail: PotDetailResponse; onJoin: () => void }) {
  ...
  return (
    <main
      aria-label="배달팟 상세"
      className="bg-muted mx-auto h-dvh max-w-[393px] overflow-hidden shadow-xl"
    >
      <div className="bg-bg flex h-full flex-col rounded-t-[34px] pt-3">
        <div className="bg-border mx-auto h-1 w-32 shrink-0 rounded-full" />
        <article className="min-h-0 flex-1 overflow-y-auto px-5 pt-8 pb-28">
            ...
            <ul className="mt-4 flex flex-wrap gap-7">
              {detail.members?.map((member, index) => (
                <li
                  ...
                  className="flex max-w-20 flex-col items-center gap-2 text-center"
                >
                  <span
                    className={`text-primary-fg flex size-8 items-center justify-center rounded-full text-xs font-bold ${index === 0 ? 'bg-primary' : 'bg-fg/40'}`}
                  >
                    {member.nickname?.charAt(0) || '?'}
                  </span>
                  <span className={`text-[11px] ${index === 0 ? 'text-primary' : 'text-muted-fg'}`}>
                    {member.nickname}
                  </span>
                </li>
              ))}
        ...
        <div className="bg-bg absolute inset-x-0 bottom-0 z-20 mx-auto max-w-[393px] px-5 pt-3 pb-[max(24px,env(safe-area-inset-bottom))]">
  ...
  return (
    ...
    >
      <div className="relative flex h-full flex-col">
        <header className="flex shrink-0 items-center justify-center px-5 pt-[max(28px,env(safe-area-inset-top))] pb-4">
        ...
        <form
          id="join-pot-form"
          onSubmit={submit}
          className="min-h-0 flex-1 overflow-y-auto px-5 pt-6 pb-28"
        >
...
function PageState({ children, error }: { children?: string; error?: string }) {
  return (
    <main
      aria-label="배달팟 상세"
      className="mx-auto flex h-dvh max-w-[393px] items-center justify-center px-5"
    >
      <p
        role={error ? 'alert' : undefined}
        className={`text-center text-sm ${error ? 'text-down' : 'text-muted-fg'}`}
      >
        {error || children}
      </p>
    </main>
