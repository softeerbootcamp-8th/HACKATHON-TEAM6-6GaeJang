...
- GitHub Actions: 프론트/백엔드 각각 별도 워크플로우로 lint+test 실행 (PR 시 트리거).
  - `.github/workflows/backend-ci.yml` — `./gradlew test` + `bootJar` (테스트는 h2라 MySQL 서비스 불필요)
  - `.github/workflows/frontend-ci.yml` — `pnpm lint` + `typecheck` + `build`
- 배포: main 브랜치 push 시 SSH + SCP로 빌드 산출물 서버에 전송, 간단 재시작 스크립트 실행.
  → **아직 미구성.** 서버 주소/시크릿이 정해지면 워크플로우를 추가한다.
